    ...
    try:
        ...
        if create_num == 0:
            for info in s_info:
                ...
                if ret["code"] == 0:
                    logging.info("成功更新DNS记录: 时间=%s, 解析线路=%s, 记录ID=%s, 值=%s" % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), line, info["recordId"], cf_ip))
                    successful_domains.add(domain)
                else:
                    logging.error("更新DNS记录失败: 时间=%s, 解析线路=%s, 记录ID=%s, 值=%s, 错误信息=%s" % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), line, info["recordId"], cf_ip, ret["message"]))
        elif create_num > 0:
            for i in range(create_num):
                ...
                if ret["code"] == 0:
                    logging.info("成功创建DNS记录: 时间=%s, 解析线路=%s, 值=%s" % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), line, cf_ip))
                    successful_domains.add(domain)
                else:
                    logging.error("创建DNS记录失败: 时间=%s, 解析线路=%s, 值=%s, 错误信息=%s" % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), line, cf_ip, ret["message"]))
        else:
            for info in s_info:
    ...
    if len(CONFIG["DOMAINS"]) > 0:
        try:
            ...
            if cfips is None or cfips["code"] != 200:
                logging.error("获取优选IP错误: 时间=%s" % time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
                return
                    ...
                    if ret["code"] == 0:
                        for record in ret["data"]["records"]:
                            if record["line"] in ["移动", "联通", "电信"]:
                                ...
                                if retMsg["code"] == 0:
                                    logging.info("成功删除DNS记录: 时间=%s, 解析线路=%s" % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), record["line"]))
                                else:
                                    logging.error("删除DNS记录失败: 时间=%s, 解析线路=%s, 错误信息=%s" % (time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), record["line"], retMsg["message"]))
